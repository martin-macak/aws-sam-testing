...
"""
        ...
        @pytest.mark.slow
        def test_run_local_api_with_dynamodb_moto(self, tmp_path: Path):
            """Test run_local_api with a Lambda function that writes to DynamoDB using Moto isolation."""
            # Create a template with API, Lambda function, and DynamoDB table
            template_content = """
AWSTemplateFormatVersion: '2010-09-09'
Transform: AWS::Serverless-2016-10-31
...
"""
            ...
            # Create source directory and Lambda handler
            src_dir = tmp_path / "src"
            src_dir.mkdir()
            
            # Create Lambda function that writes to DynamoDB
            handler_file = src_dir / "app.py"
            handler_file.write_text('''
import json
import os
...
def lambda_handler(event, context):
    """Create a new user in DynamoDB table."""
    try:
        ...
        if AWS_ENDPOINT_URL:
            # Use custom endpoint for testing with Moto
...
''')
            ...
            # Print build directory structure
            print("\nBuild directory structure for DynamoDB test:")
            TestAWSSAMToolkit.print_directory_tree(build_dir)

            try:
                # Run local API with MOTO isolation level
                with toolkit.run_local_api(isolation_level=IsolationLevel.MOTO) as apis:
                    ...
                    # Check API properties
                    assert api.api_logical_id == "UserApi"
                    assert api.port is not None
                    assert api.host is not None
                    ...
                    response = requests.post(
                        f"http://{api.host}:{api.port}/users",
                        json=user_data,
                    ...
                    assert response_data["user"]["email"] == "john.doe@example.com"
                    assert "id" in response_data["user"]
                    assert "created_at" in response_data["user"]
                    ...
                    assert response_data2["user"]["name"] == "Jane Smith"
                    assert response_data2["user"]["email"] == "jane.smith@example.com"
